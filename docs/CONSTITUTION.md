...
- The published package ships `src`, `adapters`, `catalog`, `install`, `bin`, `mcp`, `skills`, `bundled-skills`, `docs`, `tsconfig.json`, `README.md`, `NOTICE.md`, `novahiz.config.example.json`, and `LICENSE`. It never carries secrets, the local database, or machine-specific config.
