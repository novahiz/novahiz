...
description: |
  Step 6 of the skillenforce pipeline: close the gap between intent and code.
...
`skillenforce-converge` measures the gap between what was requested and what the code does. `skillenforce-audit` measures session compliance with rules. The first is about work; the second is about method.
...
- The only permitted writing is adding remainders to the ledger, each becoming a traced step: `skillenforce_task action="insert"`.
