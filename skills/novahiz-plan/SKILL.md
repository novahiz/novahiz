...
name: novahiz-plan
description: |
  Étape 1 du pipeline Novahiz : décider la direction avant d'écrire du code.
...
**Étape 1 sur 6** du pipeline `novahiz-planner`. Aucun fichier applicatif n'est modifié ici. Le livrable est un plan.

Étapes suivantes : `novahiz-clarify`, puis `novahiz-task`.
...
```
novahiz_task action="new"  title="<la demande en une phrase>"
novahiz_task action="plan" todos=[...]
```

`todowrite` donne le suivi visible, `novahiz_task` le registre durable qui survit à la compaction.
...
Le gate exige `novahiz-plan` pour `code`, `debug`, `browser`, `design-ui`, `database-supabase`, `planning`, `devops` et `data`.
...
Si des choix restent ouverts sur l'architecture, les données, le périmètre ou les tests, va d'abord à `novahiz-clarify`. Sinon, `novahiz-task` convertit le plan en tâches atomiques.
