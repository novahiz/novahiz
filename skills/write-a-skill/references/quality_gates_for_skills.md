...
## Beyond the Six: Additional Quality Dimensions

The 6 are the floor. For a mature skill library, add:
...
Skills created from external sources (MIT-licensed or public-domain) must:
...
3. Review checklist runner    (combined; all 6 review-checklist items)
...
The 6-item checklist is **binding for new skills** (any skill authored after v2.6.0 must PASS all 6 before merge). For **legacy skills** authored before this discipline was established, the same rules apply as **advisory** signals to triage, not blockers.
...
**Legacy missing "Use when" trigger (26% of repo after v2.6.1 validator fix):** the highest-impact fix because it's a 1-line edit per skill. Even legacy skills should adopt this in the next time they're touched.

**Legacy placeholder descriptions (e.g., "Migration Architect" as the only description text):** these are real bugs, not just lint failures. Fix on sight. v2.6.1 fixed 10 of these in the engineering tier.
...
- **Novahiz review checklist** (this repo): the 6-item review checklist
...
- **Skills documentation**: official skill quality guidance
