...
"References one level deep": from the skill review checklist.
...
For skills that wrap or extend other skills (like this `write-a-skill` itself, which preserves full original content + adds wrapper sections), the 100-line ceiling becomes an artifact of attribution rather than over-conditioning. Two options:
...
- **Choosing what to put in scripts/ vs references/**: see the "When scripts belong in the skill" guidance in main SKILL.md.
...
- **Novahiz write-a-skill** (this repo): the 100-line ceiling + one-level-deep rule
- **Skill structure documentation**: official skill structure documentation
- **Engineering Blog: Prompt design + context engineering**: concise context = lower hallucination + better routing
