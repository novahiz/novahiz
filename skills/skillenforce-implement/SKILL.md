...
description: |
  Step 5 of the skillenforce pipeline: write code in increments that leave the system usable.
...
Fix it, or block the step with `skillenforce_task action="block"` and a reason. Do not mask a test by disabling it, and do not bypass a failure with a silent error block.
...
Each increment produces the modified files, the executed proof, and its result. Move to `skillenforce-converge` when the slices are complete.
