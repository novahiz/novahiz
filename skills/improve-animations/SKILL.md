...
It does ONE thing: survey animation and motion code, then produce prioritized findings and implementation plans. It does not review a single diff (that's `emil-design-eng`), and it does not implement fixes itself.
...
| `execute <plan>` | Dispatch an executor subagent to implement the plan in an isolated worktree, then review its diff with the `emil-design-eng` bar and render a verdict |
