...
into that project's folder; if not, it goes to `_raw/` (staging — this vault has no `misc/` root; raw
URL pages are staged in `_raw/` and promoted later based on connection affinity). Config resolution,
the content trust boundary, and the QMD refresh step are the same as the main `wiki-ingest/SKILL.md`
— follow those; this file only covers the URL-specific mechanics.
...
4. **No project context** — if the current directory IS the obsidian-wiki repo itself, or if detection produces a name that matches the wiki vault directory, treat it as "no project context" and fall back to `_raw/`.
...
| No project context | Fall back to `_raw/` — staging (Step U3c) |
...
- If in staging mode: grep `$OBSIDIAN_VAULT_PATH/_raw/` for the URL string
...
### Step U3c: No project context (staging fallback)

Target: `$OBSIDIAN_VAULT_PATH/_raw/<slug>.md`

This vault has no `misc/` directory — `_raw/` is the staging area for source material without a
project context. It already exists at setup; do **not** create any new root directory. Do not create
a `misc/` directory. The page is promoted later (e.g. via `wiki-lint` affinity or `wiki-update`) once
it accumulates enough project connections.
...
## Step U5b: Affinity scoring (staging mode only)
...
Skip this step if in staging mode.
...
```json
{
  ...
  "promotion_status": "<project-name or staged>",
...
- Staging mode: under `## Raw / staging` (create the section at the bottom if it doesn't exist)
...
Staging mode:
```
- [TIMESTAMP] INGEST_URL url="<url>" page="_raw/<slug>.md" affinity={} promotion_status=staged mode=staging
```
...
- [ ] Page written with correct frontmatter for the mode (project vs. staging)
...
- [ ] In staging mode: `affinity` and `promotion_status` fields present
