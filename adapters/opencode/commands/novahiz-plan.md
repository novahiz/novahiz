...
1. Classify the request using `novahiz_classify`. Read the categories and required skills.
...
4. Trace the plan in the ledger with `novahiz_task`: `action: "new"` to create the task, then `action: "plan"` to deposit the steps. Do not open any step.
...
End with the honest next step: re-run the same request without the `/novahiz-plan` prefix to execute the plan.
